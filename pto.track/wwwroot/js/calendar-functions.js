...
function updateSelectedStatusesFromCheckboxes(filterElements) {
    const statuses = [];
    if (filterElements.filterPending.checked)
        statuses.push("Pending");
    if (filterElements.filterApproved.checked)
        statuses.push("Approved");
    if (filterElements.filterRejected.checked)
        statuses.push("Rejected");
    if (filterElements.filterCancelled.checked)
...
function isUserManagerOrApprover(user) {
    ...
    return user.isApprover ||
        user.roles?.some(r => r.toLowerCase() === 'manager' ||
...
export { getStatusColor, buildAbsencesUrl, determineUserRole, getDefaultStatusFilters, getVisibleFilters, updateSelectedStatusesFromCheckboxes, isUserManagerOrApprover, canCreateAbsenceForResource, getResourceSelectionMessage, buildContextMenuItems, getSchedulerRowColor, shouldAllowSelection, getCellCssClass, updateViewButtons };
