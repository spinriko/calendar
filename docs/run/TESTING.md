...
This document is a concise, high-level summary of what the repository's test and metrics suites cover and the latest snapshot of code-quality statistics. It intentionally omits run instructions (see `RUN-LOCAL.md` for how to run tests and metrics locally).

## What is tested
- Unit tests: logic in services and helper libraries across the backend and frontend.
- Integration tests: end-to-end flows that exercise controllers → services → persistence using in-memory/provider test setups.
- API behavior and edge cases: controllers, routing, error handling, and authorization flows.
- Service layer: business rules, validations, transaction handling, and user sync logic.
- Data layer: EF Core schemas, validation, and repository behaviors.
- Frontend (TypeScript): core calendar logic, filters, permissions, scheduler behavior and selected integration suites.
- Code quality metrics: a custom metrics harness computes cyclomatic complexity, maintainability index, LOC, nesting, coupling, and parameter counts across all C# projects; outputs are stored in `artifacts/metrics/`.

## Current snapshot (metrics)
Source of truth: `artifacts/metrics/code-metrics-summary.json` (generated by `pto.track.tests` metrics harness).

- Solution totals:
   - Files: 91
   - Lines: 12,424
   - Methods: 469
   - Avg cyclomatic complexity: 1.69
   - Max cyclomatic complexity: 28
   - Avg maintainability index: 58.39
   - Methods below maintainability threshold (65): 347

- Per-project highlights (average maintainability shown):
   - `pto.track`: Avg Maintainability 59.50 (Methods below threshold: 45)
   - `pto.track.data`: Avg Maintainability 60.05 (Methods below threshold: 5)
   - `pto.track.data.tests`: Avg Maintainability 59.18 (Methods below threshold: 30)
   - `pto.track.services`: Avg Maintainability 67.15 (Methods below threshold: 42)
   - `pto.track.services.tests`: Avg Maintainability 53.09 (Methods below threshold: 117)
   - `pto.track.tests`: Avg Maintainability 54.80 (Methods below threshold: 108)

Notes:
- The metrics harness reports both solution-level and per-project metrics; `code-metrics.json` contains the full per-method details and `code-metrics-summary.json` contains the compact KPIs above.
- Average maintainability values are a snapshot; several projects (particularly test projects) currently have many methods below the conservative threshold of 65. We recommend raising thresholds or creating gradual gating rules before making these metrics blocking in CI.

## What this means for CI and PRs
- We run analyzers and metrics in separate jobs so they do not block functional tests. The analyzer job publishes `artifacts/analyzers/` and `artifacts/metrics/` for reviewers to inspect.
- Because current averages show many methods under the default maintainability threshold, we recommend choosing permissive initial thresholds and tightening them over time to avoid blocking active development.
