## About This Folder

This folder contains planning, proposal, and draft documentation for future or in-progress improvements to the PTO Track project.

**How to use this folder:**
- If a document is still here, it is a draft, proposal, or not yet fully implemented.
- If a document was fully completed and moved, a link is provided below.


### Drafts and Proposals (still in progress)

- [DEPLOY-ENHANCEMENTS.md](DEPLOY-ENHANCEMENTS.md): Deploy enhancements — post-deploy checks, pipeline hardening, bundle verification, reverse proxy/PathBase guidance.
- [ADO-ENVIRONMENTS.md](ADO-ENVIRONMENTS.md): Azure DevOps Environments adoption guide for corp servers.
- [WINDOWS-AUTH-ENRICHER.md](WINDOWS-AUTH-ENRICHER.md): Windows auth debug notes, Negotiate setup, and future identity enrichment plan.
- [IDENTITY-SYNC-PLAN.md](IDENTITY-SYNC-PLAN.md): Plan to rely on pre-synced AD/ADP identity service, resolve users from local store only, and reject not-yet-synced accounts.


### Where to find completed docs

- Semantic versioning with MinVer: see [docs/run/VERSIONING.md](../run/VERSIONING.md)
- AI-assisted development workflow: see [docs/run/AI-WORKFLOW.md](../run/AI-WORKFLOW.md)
- Bundling implementation: see [docs/run/BUNDLING.md](../run/BUNDLING.md)
- Jest test conversion: see [docs/run/TYPESCRIPT.md](../run/TYPESCRIPT.md)
- Code metrics/CI gating: see [docs/run/ci-gates.md](../run/ci-gates.md)
- Jest test coverage proposals: see [docs/run/JEST_PROPOSALS.md](../run/JEST_PROPOSALS.md)
- Future patterns/architecture: see [docs/arch/FUTURE.md](../arch/FUTURE.md)
- Tech debt/metrics snapshot: see [docs/metrics/tech-debt-top-offenders-2025-12-11-013059.md](../metrics/tech-debt-top-offenders-2025-12-11-013059.md)

If you are looking for a completed or historical doc, check the appropriate permanent folder above.

---
Last updated: December 27, 2025